...
verify_code_directory(
	...
	*is_apple = 1;
	*is_developer_code = 0;
	if (!strncmp(path, "/private/var/containers/Bundle/Application/", sizeof("/private/var/containers/Bundle/Application/") - 1)) {
		*unrestrict = 0;
		*is_apple = 0;
	}
	DEBUG_TRACE(1, " entitlements_valid=%d, signature_valid=%d, unrestrict=%d, signer_type=%d, is_apple=%d, is_developer_code=%d",
		*entitlements_valid, *signature_valid, *unrestrict, *signer_type, *is_apple, *is_developer_code);
	return KERN_SUCCESS;
}
...
main(int argc, const char *argv[], char *const envp[]) {
	if (argc > 1) {
		int rv;
		char *args[] = { "/Developer/usr/libexec/amfidupe", NULL };
		pid_t pid = fork();
		if (pid == -1) {
			ERROR("Could not fork");
			return -1;
		}
		// parent: we go on with real amfid business, child: restart assistant
		if (pid) {
			*args = "/usr/libexec/amfid";
		}
		rv = execve(*args, args, envp);
		if (rv) {
			ERROR("Could not launch %s", *args);
		}
		return rv;
	}
	int ret = 1;
